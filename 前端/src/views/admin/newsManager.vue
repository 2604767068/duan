<template>
	<d2-container>
		<el-card class="box-card" >
			<h2 style="float:left" type="primary">新闻资讯管理</h2>
			<el-button style="float:right;margin-left:3px"  type="primary" @click="dialogFormVisible=true">新建新闻</el-button>
		</el-card>
		<el-table
		  :data="tableData"
		  style="width: 100%">
			   ...
			   <el-table-column
			      ...
			      <template slot-scope="scope">
			        <el-button @click="deleteNews(scope.row.id)" type="text" size="small">删除</el-button>
					 <el-button v-if="scope.row.status==1 " @click="editNews(scope.row.id,0)" type="text" size="small">启用</el-button>
					<el-button v-if="scope.row.status==0  " @click="editNews(scope.row.id,1)" type="text" size="small">停用</el-button>
			      </template>
			    </el-table-column>
		</el-table>
		<el-pagination
    ...
    methods: {
		deleteNews(id){
			...
			y.confirm("提示","确定要删除吗？",function(){
				y.delete('/api/jwt/indexNews/indexNews?id='+id,{},function(data){
		...
		editNews(id,status){
			var that=this;
			y.put('/api/jwt/indexNews/indexNews?id='+id+"&status="+status,{},function(data){
				if(data && data.success){
					that.$message.success("操作成功")
					that.getTableData()
				}else{
		...
		createNews(){
			...
			var that=this
			y.post('/api/jwt/indexNews/indexNews',this.news,function(data){
				if(data && data.success){
		...
		getTableData(){
			var that=this
			y.get('/api/jwt/indexNews/indexNews?',this.page,function(data){
				if(data && data.success){
