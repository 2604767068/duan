...
public class OpenUserRegistController extends BaseController{
	...
	public WebResult adminLogin(String username,String password) throws UnsupportedEncodingException {
		QueryWrapper<TAdmin> query=new QueryWrapper<TAdmin>();
		query.eq("username", username).eq("password", password);
		TAdmin admin=tAdminMapper.selectOne(query);
		if(admin!=null) {
			//创建用户令牌，10小时过期
			String token=JWTUtils.createToken(admin.getId(),BaseContant.LOGIN_TOKEN_EXPRISE_TIME);
			WebResult result=WebResult.success("登录成功",admin);
	...
	public WebResult login(@RequestBody TUser user) throws UnsupportedEncodingException {
		...
		if(users!=null) {
			...
			String s2 = HexUtil.encodeHexStr(password);//密码加密进行和数据库对比
			queryWrapper.eq("password", s2);
			TUser userLogin=tUserMapper.selectOne(queryWrapper);
			if(userLogin!=null) {
				//设置jwt过期时间为十个小时36000000
				String token=JWTUtils.createToken(users.getId(),BaseContant.LOGIN_TOKEN_EXPRISE_TIME);
	...
	@ApiOperation(value = "验证当前邮箱是否已经注册", notes = "")
	@PostMapping("/isRegist")
	public WebResult isRegist(String email) {
